...
export default function RepartidoresFudoClient() {
  ...
  const handleCambiarDelivery = async (waiterIdFudo: string, nuevoDeliveryId: string) => {
  ...
  const handleEliminarMapeo = async (waiterIdFudo: string) => {
    const ok = window.confirm(`¿Eliminar mapeo para waiter Fudo "${waiterIdFudo}"?`);
    if (!ok) return;
    ...
    setMapeos((prev) => prev.filter((m) => m.waiter_id_fudo !== waiterIdFudo));
  };
    ...
    <div className="p-6 space-y-8 max-w-5xl mx-auto">
      ...
      <div className="bg-white border rounded-xl shadow-sm overflow-hidden">
        <div className="px-4 py-3 border-b bg-slate-50 flex justify-between items-center">
          ...
          <span className="text-xs text-slate-400">{mapeos.length} mapeo(s)</span>
        </div>
        ...
        {mapeos.length > 0 && (
          <div className="divide-y">
            {mapeos.map((m) => (
              ...
              >
                <div className="flex-1">
                  <p className="text-xs font-semibold text-slate-500">Waiter Fudo</p>
                ...
                <div className="flex-1">
                  ...
                  <select
                    ...
                    onChange={(e) => handleCambiarDelivery(m.waiter_id_fudo, e.target.value)}
                    className="w-full border rounded px-3 py-2 text-sm bg-white"
                  >
                    {repartidores.map((r) => (
                      <option key={r.id} value={r.id}>
