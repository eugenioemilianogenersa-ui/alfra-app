...
type Order = {
  ...
  // NUEVO: viene de orders (denormalizado). STAFF lo puede ver sin RLS extra.
  delivery_nombre?: string | null;

  // Fallback viejo (vía enrich)
...
export default function AdminPedidosClient() {
  ...
  // OJO: Esto en STAFF puede no devolver nada por RLS, por eso mostramos delivery_nombre primero.
  ...
  // --- RENDER ---
  if (loading)
    return <div className="p-10 text-center text-slate-500 animate-pulse">Cargando panel de control...</div>;
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-slate-600 mb-4">
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-slate-400">🕒</span>
                      <span>
                        {new Date(p.creado_en).toLocaleString("es-AR", { hour: "2-digit", minute: "2-digit", day: "2-digit", month: "2-digit" })}
                      </span>
                    </div>
                  </div>
