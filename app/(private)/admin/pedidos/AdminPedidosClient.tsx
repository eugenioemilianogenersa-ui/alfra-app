...
export default function AdminPedidosClient() {
  ...
  const enrich = async (orders: any[], currentRepartidores: Profile[]) => {
    ...
    currentRepartidores.forEach((p) => { nameMap[p.id] = p.display_name || p.email?.split("@")[0] || "Repartidor"; });
  ...
  const syncFudo = async (forced?: boolean) => {
    ...
    finally { isSyncingRef.current = false; setSyncingFudo(false); }
  };
  ...
  const borrarPedido = async (id: number) => {
    ...
    if (!error) { setPedidos(prev => prev.filter(p => p.id !== id)); setTimeout(() => cargarPedidos(), 500); }
  };

  const asignarDelivery = async (orderId: number, deliveryUserId: string) => {
    if (!deliveryUserId) return alert("Seleccioná un repartidor");
    await fetch("/api/delivery/assign", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ orderId, deliveryUserId }) });
  ...
  const cambiarEstado = async (id: number, estado: string) => {
    ...
    try {
      await updateOrderStatus({ orderId: id, estado, source: myRole === "staff" ? "APP_STAFF" : "APP_ADMIN" });
      if (["enviado", "entregado", "cancelado"].includes(estado)) {
        await fetch("/api/push/notify-order-status", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ orderId: id, estado }) });
      }
    } catch { await cargarPedidos(); alert("Error al actualizar estado."); }
  };
  ...
  useEffect(() => {
    (async () => {
      ...
      const loadedReps = await cargarRepartidores();
      setLoading(false);
    })();
    const ch = supabase.channel("admin-live").on("postgres_changes", { event: "*", schema: "public", table: "orders" }, () => cargarPedidos()).on("postgres_changes", { event: "*", schema: "public", table: "deliveries" }, () => cargarPedidos()).subscribe();
    const i = setInterval(() => { if (document.visibilityState === "visible") syncFudo(); }, 30000);
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 pb-32 max-w-7xl mx-auto font-sans">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-100 pb-4">
        <div>
            <h1 className="text-2xl font-black text-slate-800 tracking-tight">{myRole === 'staff' ? 'Despacho de Pedidos' : 'Gestión de Pedidos'}</h1>
            <p className="text-xs text-slate-400 mt-1">{myRole === 'staff' ? 'Vista de turno actual (Fudo Mode)' : 'Panel de administración global'}</p>
        </div>
        <div className="flex items-center gap-3">
            <span className="text-[10px] font-bold tracking-wider text-emerald-600 flex items-center gap-1.5 bg-emerald-50 px-2 py-1 rounded"><span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" /> LIVE</span>
            <button onClick={() => syncFudo(true)} disabled={syncingFudo} className={`text-xs font-semibold px-4 py-2 rounded-lg border transition-all ${syncingFudo ? "bg-slate-50 text-slate-400 border-slate-200" : "bg-white text-slate-700 border-slate-200 hover:border-slate-400 hover:shadow-sm"}`}>{syncingFudo ? "Sincronizando..." : "↻ Sincronizar Fudo"}</button>
        </div>
      </div>
      ...
      {myRole === 'admin' && (
        <div className="bg-slate-900 text-white rounded-xl p-3 shadow-lg shadow-slate-200 flex flex-col xl:flex-row gap-4 items-center justify-between">
            <div className="flex flex-wrap gap-2 bg-slate-800 p-1 rounded-lg">
                <button onClick={() => setAdminMode("LIVE")} className={`px-3 py-1.5 text-[10px] md:text-xs font-bold rounded-md transition-colors ${adminMode === "LIVE" ? "bg-white text-slate-900 shadow-sm" : "text-slate-400 hover:text-white"}`}>TURNO ACTUAL</button>
                <button onClick={() => setAdminMode("ALL")} className={`px-3 py-1.5 text-[10px] md:text-xs font-bold rounded-md transition-colors ${adminMode === "ALL" ? "bg-indigo-500 text-white shadow-sm" : "text-slate-400 hover:text-white"}`}>VISIÓN GENERAL</button>
                <button onClick={() => setAdminMode("DATE")} className={`px-3 py-1.5 text-[10px] md:text-xs font-bold rounded-md transition-colors ${adminMode === "DATE" ? "bg-white text-slate-900 shadow-sm" : "text-slate-400 hover:text-white"}`}>POR FECHA</button>
                <button onClick={() => setAdminMode("ID")} className={`px-3 py-1.5 text-[10px] md:text-xs font-bold rounded-md transition-colors ${adminMode === "ID" ? "bg-white text-slate-900 shadow-sm" : "text-slate-400 hover:text-white"}`}>BUSCAR ID</button>
            </div>
            <div className="flex-1 flex justify-end w-full md:w-auto">
                ...
                {adminMode === "DATE" && <input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="bg-slate-800 border border-slate-700 text-white text-sm rounded px-3 py-1.5 focus:outline-none" />}
                {adminMode === "ID" && <div className="flex gap-2 w-full md:w-auto"><input value={searchId} onChange={(e) => setSearchId(e.target.value)} placeholder="# ID" className="bg-slate-800 border border-slate-700 text-white text-sm rounded px-3 py-1.5 w-full md:w-32 focus:outline-none" onKeyDown={(e) => e.key === 'Enter' && cargarPedidos()} /><button onClick={() => cargarPedidos()} className="bg-white text-slate-900 px-3 py-1.5 rounded text-xs font-bold hover:bg-slate-200">BUSCAR</button></div>}
            </div>
        </div>
      )}
      ...
      <div className="grid gap-4">
        {pedidos.length === 0 ? (
            <div className="text-center py-20 bg-slate-50 rounded-xl border border-dashed border-slate-200"><p className="text-slate-400 font-medium">No se encontraron pedidos.</p>{myRole === 'staff' && <p className="text-xs text-slate-300 mt-2">Esperando pedidos del turno actual...</p>}</div>
        ) : (
            pedidos.map((p) => (
            <div key={p.id} className={`bg-white border rounded-xl p-4 md:p-5 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden ${estadoLeftBorder(p.estado)}`}>
                <div className="flex flex-wrap justify-between items-start gap-3 mb-3">
                    <div className="flex items-center gap-2"><span className="bg-slate-800 text-white px-2 py-0.5 rounded text-xs font-mono font-bold">#{p.id}</span><h3 className="font-bold text-slate-800 text-lg leading-tight">{p.cliente_nombre || "Cliente Anónimo"}</h3></div>
                    <div className="flex items-center gap-2"><span className={`px-3 py-1 text-[11px] font-bold uppercase tracking-wide rounded-full border ${estadoBadgeClass(p.estado)}`}>{p.estado}</span>{myRole === 'admin' && (<button onClick={() => borrarPedido(p.id)} className="ml-2 text-slate-300 hover:text-red-600 transition-colors p-1" title="Eliminar"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/></svg></button>)}</div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-slate-600 mb-4">
                    ...
                    <div className="flex flex-col gap-1 md:items-end">
                        ...
                        {p.estado_source && <div className="text-[10px] text-slate-400 bg-slate-50 px-2 py-0.5 rounded border border-slate-100 inline-block">Fudo Status: {p.estado_source}</div>}
                    </div>
                </div>
                ...
                <div className="pt-4 border-t border-slate-50 flex flex-wrap gap-3 items-center justify-between bg-slate-50/50 -mx-5 -mb-5 p-4 mt-2">
                    <div className="flex items-center gap-2 bg-white border rounded-lg p-1 shadow-sm">
                        <div className="px-2">
                            ...
                            <span className={`text-sm font-bold ${p.repartidor_nombre ? 'text-slate-800' : 'text-orange-500'}`}>{p.repartidor_nombre || "Sin Asignar"}</span>
                        </div>
                        <select id={`sel-rep-${p.id}`} className="text-xs bg-slate-100 border-none rounded py-1 pl-2 pr-6 focus:ring-0 cursor-pointer" defaultValue="" onChange={(e) => { if(e.target.value) asignarDelivery(p.id, e.target.value); }}>
                            <option value="" disabled>Cambiar...</option>
                            {repartidores.map((r) => (<option key={r.id} value={r.id}>{r.display_name || r.email}</option>))}
                        </select>
                    </div>
                    <div className="flex items-center gap-2">
                        <span className="text-[10px] text-slate-400 font-bold uppercase hidden md:inline">Estado:</span>
                        <select value={p.estado ?? "pendiente"} onChange={(e) => cambiarEstado(p.id, e.target.value)} className="text-xs font-medium py-1.5 pl-3 pr-8 rounded-lg border-slate-200 bg-white shadow-sm focus:border-slate-800 focus:ring-0 cursor-pointer">
